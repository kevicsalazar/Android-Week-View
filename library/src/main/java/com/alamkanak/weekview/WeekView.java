...
public class WeekView extends View {
    ...
    private boolean fitOnScreen = false;
    private boolean yScrollingEnabled = true;
    private String headerRowDateFormat;
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            switch (mCurrentFlingDirection) {
                case LEFT:
                case RIGHT:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, (int) (velocityX * mXScrollingSpeed), 0, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * (maxHour - minHour) + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
                case VERTICAL:
                    if (yScrollingEnabled) {
                        mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, 0, (int) velocityY, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * (maxHour - minHour) + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    }
                    break;
            }
    ...
    public WeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            zoomEnabled = a.getBoolean(R.styleable.WeekView_zoomEnabled, zoomEnabled);
            fitOnScreen = a.getBoolean(R.styleable.WeekView_fitOnScreen, fitOnScreen);
            yScrollingEnabled = a.getBoolean(R.styleable.WeekView_yScrollingEnabled, yScrollingEnabled);
            headerRowDateFormat = a.getString(R.styleable.WeekView_headerRowDateFormat);
    ...
    private void initTextTimeWidth() {
        ...
        for (int i = 0; i < (maxHour - minHour); i++) {
            ...
            String time = getDateTimeInterpreter().interpretTime(i + minHour);
            if (time == null)
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (fitOnScreen) {
            mHourHeight = getHeight() / (maxHour - minHour + 1);
        }
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mAreDimensionsInvalid) {
            mEffectiveMinHourHeight = Math.max(mMinHourHeight, (int) ((getHeight() - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom) / (maxHour - minHour)));
        ...
        if (mCurrentOrigin.y < getHeight() - mHourHeight * (maxHour - minHour) - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight / 2)
            mCurrentOrigin.y = getHeight() - mHourHeight * (maxHour - minHour) - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight / 2;
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            int i = 0;
            for (int hourNumber = 0; hourNumber < maxHour - minHour; hourNumber++) {
            ...
            if (mShowNowLine && sameDay) {
                float startY = mHeaderHeight + mHeaderRowPadding * 2 + mTimeTextHeight / 2 + mHeaderMarginBottom + mCurrentOrigin.y;
                Calendar now = Calendar.getInstance();
                float beforeNow = (now.get(Calendar.HOUR_OF_DAY) - minHour + now.get(Calendar.MINUTE) / 60.0f) * mHourHeight;
        ...
        for (int dayNumber = leftDaysWithGaps + 1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            if (mNumberOfVisibleDays == 1) {
                canvas.drawText(dayLabel, (startPixel + mWidthPerDay / 2) - (mTimeTextWidth + mHeaderColumnPadding * 2) / 2, mHeaderTextHeight + mHeaderRowPadding, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
            } else {
    ...
    private float calculateTimeToPixel(float bottom) {
        return mHourHeight * (bottom / 60) - minHour * mHourHeight + mCurrentOrigin.y + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2;
    }
    ...
    private void drawEventTitle(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        ...
        StaticLayout locationLayout = null;
        ...
        if (event.getLocation() != null) {
            if (!event.getLocation().contains("\n")) {
                ...
                locationLayout = new StaticLayout(new SpannableStringBuilder(event.getLocation().replace("\n", "")), mEventTextPaint, availableWidth, Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
            }
        }
        ...
        if (locationLayout != null) {
            canvas.save();
            canvas.translate(originalLeft + mEventPadding, originalTop + mEventPadding + textLayout.getHeight());
        ...
        if (mDateTimeInterpreter == null) {
            mDateTimeInterpreter = new DateTimeInterpreter() {
                ...
                public String interpretDate(Calendar date) {
                    try {
                        String format = headerRowDateFormat != null ? headerRowDateFormat : "EEE dd/MM/yyyy";
                        SimpleDateFormat sdf = new SimpleDateFormat(format, Locale.getDefault());
                        return capitalize(sdf.format(date.getTime()));
                    } catch (Exception e) {
                        e.printStackTrace();
                        return "";
                    }
    ...
    public void goToHour(double hour) {
        ...
        if (hour > (maxHour - minHour))
            verticalOffset = mHourHeight * (maxHour - minHour);
        else if (hour > 0)
            verticalOffset = (int) (mHourHeight * hour);

        if (verticalOffset > mHourHeight * (maxHour - minHour) - getHeight() + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)
            verticalOffset = (int) (mHourHeight * (maxHour - minHour) - getHeight() + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom);
    ...
    public interface EventLongPressListener {
        /**
         * Similar to  but with a long press.
         *
         * @param event:     event clicked.
         * @param eventRect: view containing the clicked event.
         */
    ...
    public interface EmptyViewLongPressListener {
        /**
         * Similar to  but with long press.
         *
